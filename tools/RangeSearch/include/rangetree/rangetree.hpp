...
private:
    ...
    void searchR (const Range<T, OrigD>& search_range, std::vector<Point<T, OrigD>>& out) const {
        if (root == nullptr) {
            return;
        }
        ...
        auto collect_subtree = [&](Node* n) {
            if (n == nullptr) return;
            n->next_tree.searchR(search_range, out);
        };
...
private:
    ...
    void searchR(const Range<T, OrigD>& search_range, std::vector<Point<T, OrigD>>& out) const {
        ...
        out.insert(out.end(), it1, it2);
    }

public:
    RangeTreeNd(const std::vector<Point<T, OrigD>>& base_points) {
        sorted_points = std::move(base_points);
...
template class rangetree::RangeTreeNd<double, 1>;
