...
// Max compile-time supported dimension. Bump and recompile if you need more.
inline constexpr std::size_t kMaxDim = 12;

using RawPoints = std::vector<std::vector<double>>;
using RawRange  = std::vector<std::pair<double,double>>; // {start,end} per axis
...
// ---------- boost::rtree wrapper: build + search only ----------
...
// ---------- generic runner for a fixed T,K ----------
template <typename T, std::size_t K>
...
// ---------- runtime dispatch table over K = 1..kMaxDim for a fixed T ----------
...
constexpr std::array<RunFn, sizeof...(Ks)> makeTable(std::index_sequence<Ks...>) {
    return { &runSearch<T, Ks + 1>... }; // K = Ks+1 -> covers 1..kMaxDim
}
